...
- Не добавляй вводных/завершающих фраз от лица модели. Запрещены, в том числе:
  ...
  · мотивационные вступления: «Сегодня мы вспомним…», «Это очень важный навык…», «Давай повторим и закрепим…»;
  · мета-задания без конкретной задачи: «Вспомни правило … и запиши его», «Вспомни определение…», «Расскажи, что ты знаешь о…».
- В intro и question сразу давай учебный материал или конкретную задачу. Хорошие образцы:
  · intro: «Сегодня потренируемся умножать обыкновенные дроби. Считай внимательно и проверяй сокращение.» (без «Привет!»);
  · question: «Чему равно произведение дробей $\\frac{2}{3} \\cdot \\frac{1}{5}$?»;
  · не так: «Вспомни правило умножения обыкновенных дробей и запиши его.»
...
Верни JSON:
{
  ...
  "intro": "2–3 предложения по теме на «ты», без приветствий и без «вспомни правило…» (или пустая строка, если intro не нужен)",
